...
void fixheap(vector<Player>& v1, int vacant, Player K, int N, bool isSort) {//fixheap 구현(construct heap용)
	if (vacant * 2 > N) {    //vacant가 리프인 경우,N이 마지막 유효한 인덱스이므로 자식이 N보다 크면 리프다
		...
		if (v1[smallerChild].record > K.record) {//K가 더 작다면 vacant에 K대입
			...
			if (isSort) TotalTime += 1;//vacant의 깊이가 1칸 밑으로 이동,정렬할때만 바뀌도록 설정
...
void constructHeap(vector<Player>& v1, int root, int N) {
	if (root * 2 <= N) {//자식이 있는지 확인,있을때만 실행
		...
		fixheap(v1, root, K, N, false);
	}
}

int getHeight(int N) {//힙의 높이 구하는 함수,이진 탐색 최악의 경우 조건 참고
	int h = 0;
...
void bubbleUpHeap(vector<Player>& v1, int vacant, Player K) {//처음 이동한 vacant에서의 값을 비교
	if (vacant == 1) {//vacant가 루트에 위치하는 경우, vacant인덱스에 K를 넣어주고 끝
		...
		if (K.record >= v1[vacparent].record) {//vacparent가 더 작은 경우, 현재 위치에 K대입
			...
			TotalTime += 1;//vacant의 깊이가 1칸 위로 이동
			bubbleUpHeap(v1, vacparent, K);
		}
	}
}


int promote(vector<Player>& v1, int hStop, int vacant, int h, int N) {//vacant의 위치를 한번에 낮춰주는 함수
	int vacStop;
	if (h <= hStop || vacant * 2 > N) {//목표 높이에 도달했거나 자식이 없으면 정지
		...
		if (v1[2 * vacant].record <= v1[2 * vacant + 1].record) {//왼쪽이 더 작을 때
			...
			TotalTime += 1;//vacant의 깊이가 1칸 밑으로 이동
			vacStop = promote(v1, hStop, 2 * vacant, h - 1, N);
		}
		else {//오른쪽이 더 작을 때
			v1[vacant] = v1[2 * vacant + 1];
			TotalTime += 1;//vacant의 깊이가 1칸 밑으로 이동
			vacStop = promote(v1, hStop, 2 * vacant + 1, h - 1, N);
		}
	}
	return vacStop;
}
...
void fixHeapFast(vector<Player>& v1, int vacant, Player K, int h, int N) {//heapsort용 fixheapfast
	if (h <= 1) {//높이가 1 이하인 경우,promote를 사용하더라도 어차피 한층의 두번의 비교가 필요,따라서 직접 비교로 불필요한 계산을 줄인다
		fixheap(v1, vacant, K, N, true);
	}
	else {
		int hStop = h / 2;//한번에 내릴 목표 설정
		int vacStop = promote(v1, hStop, vacant, h, N);//promote함수를 통해 vacStop인덱스를 구함
		...
		if (v1[vacParent].record > K.record) {//부모가 K보다 큰경우
			...
			fixHeapFast(v1, vacStop, K, hStop, N);
		}
	}
}
